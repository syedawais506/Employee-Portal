...
```
then check **Notifications** (bell icon) as `employee@acme-demo.com` for a "Please submit your timesheet" entry once their last submission goes stale past the threshold. Unlike the work-anniversary/document-expiry reminders below, this one re-fires every day the employee stays stale, not just once.
