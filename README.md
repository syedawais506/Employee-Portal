...
Access is two-tiered: log back in as `admin@acme-demo.com` and ask **Ask HR** something like *"Who has pending leave requests?"* or *"What's everyone's leave balance?"* — Admin sees the full company-wide picture (every employee's balance, the whole pending queue by name). Log in as `employee@acme-demo.com`, `manager@acme-demo.com`, or `hr@acme-demo.com` and ask the same thing — none of them can see anything beyond their own balance, structurally (the query for anyone else's data is never even run for a non-Admin request, not just withheld by the prompt).
