...
- PostgreSQL **Row-Level Security (RLS)** policies are enabled as defense-in-depth: the app sets `app.current_company_id` per request/transaction (`app/db/rls.py`), and RLS policies on tenant tables reject rows outside that value even if application code has a bug.
...
> **RLS is genuinely enforced, not just present.** Postgres RLS never applies to superusers or roles with `BYPASSRLS`, and (without `FORCE ROW LEVEL SECURITY`, which every RLS migration sets) not to the table owner either. Docker Compose provisions two distinct roles specifically to avoid this trap: `POSTGRES_USER` (a superuser, since that's unavoidable for whichever role owns the schema) runs migrations only, via `MIGRATION_DATABASE_URL`; the app itself (backend, celery-worker, celery-beat) connects at runtime as `APP_DB_USER` — a restricted, non-superuser, non-`BYPASSRLS` role created by `docker/postgres-initdb/01-create-app-role.sh` (baked into a small custom Postgres image at `docker/postgres/Dockerfile`, since Docker Desktop's bind-mount sharing can refuse host paths under `~/Desktop` on macOS even though build contexts are unaffected) — with only DML grants (`SELECT`/`INSERT`/`UPDATE`/`DELETE`), no ownership. RLS on this role's queries is real, not inert.
>
> One table, `onboarding_invite`, is deliberately RLS-*exempt* (migration 0019): it's looked up by an unguessable token hash before the caller's `company_id` is known at all — the same structural reason `user_account` and `company` are exempt (both are also looked up before a company context exists). The token itself is the access control for that table, not `company_id`.
>
> `set_tenant_context` uses `set_config(..., false)` (session-scoped), not `set_config(..., true)`/`SET LOCAL` (transaction-scoped) — a single request or script often spans multiple `db.commit()` calls on the same connection, and `SET LOCAL`'s value is wiped at every commit, silently dropping tenant context for anything queried afterward. Session-scoped `set_config` persists across commits for the life of that pooled connection checkout; the next request to reuse the connection always calls `set_tenant_context` again (via `get_current_user`) before its own first query, so there's no cross-request leakage.
