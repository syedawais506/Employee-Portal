...
Every subclass (`EmployeeRepository`, `DepartmentRepository`, `RoleRepository`) inherits this — there is structurally no method that queries without `company_id`. Combined with Postgres RLS (`app/db/rls.py` sets `SET LOCAL app.current_company_id = :cid` at the start of each request's transaction, and each tenant table has `CREATE POLICY ... USING (company_id = current_setting('app.current_company_id')::uuid)`), a bug that forgets the filter is still blocked at the database level.
