...
- A separate **Drafts** tab is the review-and-submit step: its own period navigator, a table of every entry in that period (status, rejection reason if applicable, edit/delete while still draft/rejected), and the single **Submit Period** action — splitting "log time" from "commit to submitting it" so a half-filled week is never accidentally sent for approval
