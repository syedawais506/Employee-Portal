...
- My Timesheet UI is a full month calendar (not a flat list): the active submission period is outlined inside the month for context, clicking a date inside it opens the log-time dialog (saved as a draft immediately — no separate "draft" state to manage), and the whole period is reviewed at a glance before one **Submit Period** action
