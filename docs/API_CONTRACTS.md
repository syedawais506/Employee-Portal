...
- **Admin** (`company.configure` holder): the full company-wide picture, end to end, across every operational module — not just Leave:
  - Leave: every active employee's current-year balance (`LeaveService.list_company_balances`), the full pending-request queue by name (`LeaveService.list_requests(status="pending")`), and the dashboard aggregate.
  - Timesheets: the pending/rejected/late/billable-% dashboard (`TimesheetService.get_dashboard`) and the full pending-submission queue by name (`TimesheetService.list_submissions(bucket="pending")`).
  - Attendance: every active employee's today check-in status (`AttendanceService.today_dashboard`) — checked in, checked out, late, or not checked in yet.
  - Assets: inventory counts by status (`AssetService.get_summary`) and exactly who currently holds each assigned asset (`AssetService.list_assets(status="assigned")`).
- **Everyone else** (Employee, Manager, HR, Finance): only their own current-year leave balance (`LeaveService.list_my_balances`) — nothing from Timesheets, Attendance, or Assets is fetched at all, and no other employee's name, balance, or request ever enters the prompt, structurally (not just via prompt instruction) — those queries are simply never run for a non-Admin caller. Holidays are also filtered to the caller's own `employee.location` (Admin sees every holiday regardless of location scope).

Two different system prompts are used (`ADMIN_SYSTEM_PROMPT` / `EMPLOYEE_SYSTEM_PROMPT` in `ai_chatbot_service.py`) so the model's own framing matches what it was actually given, and — for the employee prompt — is told to say "ask an Admin" rather than guess if asked about someone else or about a module outside its scope, even though that data was never in its context to begin with.
