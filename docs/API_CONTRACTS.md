...
- **Admin** (`company.configure` holder): full company-wide leave picture — every active employee's current-year balance (`LeaveService.list_company_balances`), the full pending-request queue across the whole company (`LeaveService.list_requests(status="pending")`), and the dashboard aggregate (pending count, on-leave-today count).
- **Everyone else** (Employee, Manager, HR, Finance): only their own current-year leave balance (`LeaveService.list_my_balances`) — no other employee's name, balance, or request ever enters the prompt, structurally (not just via prompt instruction) — the query that would fetch it is simply never run for a non-Admin caller. Holidays are also filtered to the caller's own `employee.location` (Admin sees every holiday regardless of location scope).

Two different system prompts are used (`ADMIN_SYSTEM_PROMPT` / `EMPLOYEE_SYSTEM_PROMPT` in `ai_chatbot_service.py`) so the model's own framing matches what it was actually given, and — for the employee prompt — is told to say "ask an Admin" rather than guess if asked about someone else, even though that data was never in its context to begin with.
